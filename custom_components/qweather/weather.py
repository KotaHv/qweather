...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    _LOGGER.info("设置和风天气ing")
    weather_entity = QWeather(
        config[CONF_API_KEY], config[CONF_REGION], config[CONF_NAME]
    )
    async_add_entities([weather_entity], True)


class QWeather(WeatherEntity):
    def __init__(self, api_key: str, location: str, name: str) -> None:
        super().__init__()
        ...
        self.daily_url = f"https://devapi.qweather.com/v7/weather/3d?location={self.location}&key={self.api_key}"
    ...
    async def async_update(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(self.daily_url) as response:
                self._daily_data = (await response.json())["daily"]
